...
const STORAGE_KEY = "schedule_events_v2";
      ...
      <div style={{ background: "#12121C", border: "1px solid #2A2A40", borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 400, boxShadow: "0 20px 60px #00000099" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
          <div style={{ flex: 1, background: "#1a1035", border: "1px solid #A29BFE33", borderRadius: 14, padding: "16px 14px", textAlign: "center" }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: "#A29BFE", lineHeight: 1 }}>
          ...
          <div style={{ flex: 1, background: "#0a1f1a", border: "1px solid #55EFC433", borderRadius: 14, padding: "16px 14px", textAlign: "center" }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: "#55EFC4", lineHeight: 1 }}>
        ...
        <div style={{ marginBottom: 8 }}>
              ...
              return (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, height: "100%" }}>
                  <div style={{ flex: 1, display: "flex", alignItems: "flex-end", width: "100%" }}>
                    <div style={{
                      width: "100%",
                      height: `${Math.max(h, d.mins > 0 ? 4 : 0)}%`,
                      background: d.isToday
                        ? "linear-gradient(180deg,#A29BFE,#6C5CE7)"
                        : d.mins > 0 ? "#A29BFE44" : "#1E1E30",
                      borderRadius: "4px 4px 2px 2px",
                      transition: "height 0.5s ease",
                      position: "relative",
                    }}>
                      {d.mins > 0 && (
                        <div style={{ position: "absolute", top: -18, left: "50%", transform: "translateX(-50%)", fontSize: 9, color: d.isToday ? "#A29BFE" : "#6B6B8A", whiteSpace: "nowrap", fontWeight: 600 }}>
                          {d.mins}m
                        </div>
                      )}
                    </div>
                  </div>
                  <div style={{ fontSize: 9, color: d.isToday ? "#A29BFE" : "#3A3A5A", fontWeight: d.isToday ? 700 : 400, whiteSpace: "nowrap" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
            {data.map((d, i) => (
              <div key={i} style={{ flex: 1, textAlign: "center", fontSize: 9, color: d.isToday ? "#A29BFE" : "#3A3A5A", fontWeight: d.isToday ? 700 : 400 }}>
                {d.label.split("(")[0]}
              </div>
            ))}
          </div>
...
function MonkOverlay({ event, onClose }) {
  ...
  const sessionFocusRef = useRef(0); // 今回起動からの集中秒数
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: isFocus ? "radial-gradient(ellipse at 50% 30%,#1a0a2e,#0A0A0F 70%)" : "radial-gradient(ellipse at 50% 30%,#0a1f1a,#0A0A0F 70%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", transition: "background 1s" }}>
      <div style={{ marginBottom: 12, fontSize: 11, letterSpacing: 4, textTransform: "uppercase", color: isFocus ? "#A29BFE" : "#55EFC4", fontWeight: 700 }}>
        {isFocus ? "🧘 MONK MODE" : "☕ BREAK TIME"} — Round {round}
      </div>
...
export default function App() {
  ...
  const [form, setForm] = useState({ title: "", start: "08:00", end: "09:00", colorIdx: 0, monkMode: false, focusMins: 25, breakMins: 5 });
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      setTodayFocus(loadLog()[todayKey()] || 0);
    }, 1000);
  ...
  const openAdd = () => { setEditingEvent(null); setForm({ title: "", start: "08:00", end: "09:00", colorIdx: 0, monkMode: false, focusMins: 25, breakMins: 5 }); setShowModal(true); };
  const openEdit = (ev) => { setEditingEvent(ev); setForm({ title: ev.title, start: ev.start, end: ev.end, colorIdx: ev.colorIdx, monkMode: ev.monkMode || false, focusMins: ev.focusMins || 25, breakMins: ev.breakMins || 5 }); setShowModal(true); };
  const saveEvent = () => {
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
            <div style={{ display: "flex", gap: 8 }}>
              {notifPermission !== "granted" && (
                <button onClick={requestNotif} style={{ background: "#1E1E30", border: "1px solid #FF6B6B44", color: "#FF6B6B", borderRadius: 10, padding: "8px 12px", fontSize: 12, cursor: "pointer" }}>🔔</button>
              )}
            ...
            <button onClick={() => setShowStats(true)} style={{
              ...
              background: todayFocus > 0 ? "#1a1035" : "#1a1a2a",
              border: `1px solid ${todayFocus > 0 ? "#A29BFE44" : "#2A2A40"}`,
              borderRadius: 10, padding: "6px 12px", cursor: "pointer",
              transition: "all 0.3s",
            }}>
              <span style={{ fontSize: 13 }}>🧘</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: todayFocus > 0 ? "#A29BFE" : "#3A3A5A" }}>
                {todayFocus > 0 ? focusLabel : "0m"}
              </span>
        ...
        <div style={{ position: "relative", paddingLeft: 64, paddingRight: 20, minHeight: 1440 }}>
          {hours.map((h) => (
            <div key={h} style={{ position: "absolute", top: `${(h / 24) * 100}%`, left: 0, right: 0, display: "flex", alignItems: "flex-start", pointerEvents: "none" }}>
              <span style={{ width: 52, textAlign: "right", paddingRight: 10, fontSize: 11, color: h === currentTime.getHours() ? "#A29BFE" : "#3A3A5A", lineHeight: "14px", userSelect: "none" }}>
                {h < 24 ? `${pad(h)}:00` : ""}
              </span>
      ...
      {sortedEvents.length > 0 && (
        <div style={{ background: "#0E0E1A", borderTop: "1px solid #1E1E2E", padding: "16px 20px", maxHeight: "28vh", overflowY: "auto" }}>
          ...
          {sortedEvents.map((ev) => {
            ...
            return (
              <div key={ev.id} onClick={() => ev.monkMode && isNow ? setActiveMonk(ev) : openEdit(ev)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", marginBottom: 6, borderRadius: 10, background: isNow ? "#1A1A2E" : "transparent", cursor: "pointer", opacity: isPast ? 0.45 : 1, border: isNow ? `1px solid ${ev.monkMode ? "#A29BFE33" : color.bg + "33"}` : "1px solid transparent" }}>
                <div style={{ width: 3, height: 32, borderRadius: 2, background: ev.monkMode ? "#A29BFE" : color.bg, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: isPast ? "#6B6B8A" : "#E8E8F0", display: "flex", alignItems: "center", gap: 6 }}>{ev.monkMode && <span style={{ fontSize: 12 }}>🧘</span>}{ev.title}</div>
                  <div style={{ fontSize: 11, color: "#6B6B8A" }}>{ev.start} – {ev.end}{ev.monkMode ? ` · 集中${ev.focusMins}分 休憩${ev.breakMins}分` : ""}</div>
                </div>
